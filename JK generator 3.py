...
for count in range(len(wide_Y)):
    if not np.random.randint(2): style_Y+=pattern_stripe_nonuniform(wide_Y.pop(np.random.randint(len(wide_Y))),np.random.randint(20,40)/100,np.random.randint(25,400)/100,np.random.randint(2,6),colorModes[np.random.randint(3)],basicColor,rand_color(optionalColors))
