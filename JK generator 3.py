...
def add_wide(n=3):
    ...
    while flag:
        ...
        for i in range(n):
            if not 0.2*unit<(pos0[2*i+1]-pos0[2*i])/size<2.4*unit: flag=True
        for i in range(1,n):
            if not 0.1*unit<(pos0[2*i]-pos0[2*i-1])/size<0.8*unit: flag=True
        if not pos0[0]/size<0.2*unit: flag=True
        if not (size-pos0[-1])/size<0.2*unit: flag=True
...
for count in range(len(wide_X)):
    if not np.random.randint(3+count): style_X+=pattern_stripe_nonuniform(wide_X.pop(np.random.randint(len(wide_X))),np.random.randint(40,80)/100,np.random.randint(25,400)/100,np.random.randint(1,4),colorModes[np.random.randint(2)],basicColor,rand_color(optionalColors))
...
for multiline in multiline_X:
    if np.random.randint(4): style_X+=pattern_stripe_uniform(multiline,int(((multiline[1]-multiline[0])/(size/25)+1)/2),rand_color(optionalColors))
for thread in thread_X:
    if np.random.randint(4): style_X+=pattern_stripe_uniform(thread,int(((thread[1]-thread[0])/(size/100)+1)/2),threadColor)

for count in range(len(background_Y)):
    if not np.random.randint(2+count): style_Y+=pattern_smooth(background_Y.pop(np.random.randint(len(background_Y))),0,basicColor,rand_color(optionalColors))
for count in range(len(wide_Y)):
    if not np.random.randint(2+count): style_Y+=pattern_stripe_nonuniform(wide_Y.pop(np.random.randint(len(wide_Y))),np.random.randint(40,80)/100,np.random.randint(25,400)/100,np.random.randint(1,4),colorModes[np.random.randint(2)],basicColor,rand_color(optionalColors))
for count in range(len(narrow_Y)):
    if not np.random.randint(2+count): style_Y+=pattern_stripe_nonuniform(narrow_Y.pop(np.random.randint(len(narrow_Y))),np.random.randint(0,40)/100,np.random.randint(25,400)/100,np.random.randint(1,4),colorModes[np.random.randint(2)],rand_color(optionalColors),rand_color(optionalColors))
