...
def add_wide(n=3):
    ...
    while flag:
        ...
        for i in range(n):
            if not 0.2*unit<(pos0[2*i+1]-pos0[2*i])/size<2*unit: flag=True
        for i in range(1,n):
            if not 0.1*unit<(pos0[2*i]-pos0[2*i-1])/size<1*unit: flag=True
        if not 0.2*unit<pos0[0]/size<0.6*unit: flag=True
        if not 0.2*unit<(size-pos0[-1])/size<0.6*unit: flag=True
